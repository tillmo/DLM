...
class MiningTrainerBase(CurriculumTrainerBase):
  ...
  def _get_number_and_player(self, meters, mode, index):
    ...
    if balanced_sample:
      ...
      player = self._get_player(number, mode, index)
    if mode == 'train':
  ...
  def _get_result(self, index, meters, mode):
    number, player = self._get_number_and_player(meters, mode, index)
  ...
  def advanced_test(self, inter=True, deter=False):
    mode_ = 'test-inter' if inter else 'test'
    if deter:
        mode_ += '-deter'
    self.model.eval()
    results = []
    ...
    self.test_number = self.test_number_begin
    while self.test_number <= self.test_number_end:
        meters = self._advanced_test_epoch(self.test_epoch_size, mode_)
        results.append(meters)
        if self.test_number_step <= 0:
            break
        self.test_number += self.test_number_step
  ...
  def _advanced_test_epoch(self, epoch_size, mode_):
    meters = GroupMeters()
    self._prepare_dataset(epoch_size, mode=mode_)

    def test_func(index):
      message, _ = self._get_result(index, meters, mode=mode_)
      return message

    tqdm_for(epoch_size, test_func)
    logger.info(meters.format_simple('> Evaluation (%s): ' % mode_, compressed=False))
    self._dump_meters(meters, mode_)
    return meters
