...
MiningTrainerBase.make_trainer_parser(
    parser, {
        ...
        'curriculum_thresh_relax': 0.0, #want interpretable at each lesson
        'curriculum_thresh': 1,
...
class Model(nn.Module):
    ...
    def __init__(self, concat_worlds, distribution, nlm_breadth):
        ...
        self.isQnet = True
        self.best_score = np.array([0.0, 0.0])
        self.lowernoise()
    ...
    def stoch_decay(self, lesson, mining_meters):
        optimizer = None
        if args.model == 'dlm':
            ...
            if self.tau < 0.45 and mining_meters is not None:
                current_scores = np.array([mining_meters[1].avg['succ'], mining_meters[2].avg['succ']])
                if (current_scores >= 1.0).any():  # don't want to extract if it's "perfect"
                    logger.info("great, improved to " + str(current_scores) + " but don't trigger extract here")
                else:
                    entries_size = np.array(self.features.input_dims).sum()
                    logger.info("TRIGGERED " + str(sum(p.numel() for p in self.features.parameters())) +
                                " parameters with " + str(entries_size) + " predicates")
                    self.tau = args.tau_begin
                    self.dropout_prob = args.dropout_prob_begin
                    self.gumbel_prob = args.gumbel_noise_begin
                    if (self.best_score < current_scores).any():  # learned something better
                        logger.info("improved " + str(self.best_score) + " to " + str(current_scores))
                        new_features, entries_size = self.features.extract_stacked(self.feature_axis, self.pred, onlyLastLayer=args.incr_llonly,
                        ...
                        new_pred = DLMInferenceBase(new_features.output_dims[self.feature_axis], 1, False, 'root')
                        new_pred.to(next(self.parameters()).device)
                        self.pred = new_pred
                        self.features = new_features
                    else:
                        logger.info("didn't improved with " + str(current_scores) + ", retry ...")
                        self.pred.reset_weights()
                        self.features.reset_weights()
                    optimizer = get_optimizer(args.optimizer, self, args.lr)
                    logger.info("TRIGGERED AFTER " + str(sum(p.numel() for p in self.features.parameters())) +
                               " parameters with " + str(entries_size) + " predicates")

                    self.best_score = np.maximum(self.best_score, current_scores)
...
def run_episode(env,
    ...
    if args.model == 'dlm':
        ...
        if ('inter' in mode) or ('mining' in mode):
            model.lowernoise()
    ...
    if dump_play and not (args.dump_fail_only and succ):
        ...
        json_str = json.dumps(
            ...
            dict(array=array, moves=moves, new_pos=new_pos,
                 policies=policies))
        dump_file = os.path.join(args.current_dump_dir, '{}_blocks{}.json'.format(play_name, env.unwrapped.nr_blocks))
...
#            print("warning, printed number of blocks in test isn't correct")
    ...
    def _upgrade_lesson(self):
        ...
        if not (self.is_candidate or self.is_graduated):
            self.model.best_score = np.array([0.0, 0.0])
    ...
    def _train_epoch(self, epoch_size):
        ...
        logger.info(meters.format_simple('> Train Epoch {:5d}: '.format(self.current_epoch), compressed=False))
        self._dump_meters(meters, 'train')
        if not self.is_graduated:
            mining_meters = self._take_exam(train_meters=copy.copy(meters))

            optim = self.model.stoch_decay(self.current_number, mining_meters)
            if optim:
...
def main(run_id):
    ...
    if args.load_checkpoint is not None:
        ...
        # trainer.model.features.extract_graph(2, trainer.model.pred)
        # trainer.model.features.fake_forward()
        # trainer.model.features.display_distribution_extracted(trainer.model.pred, trainer.model.tau)
